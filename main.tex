...
% Streustrahlenraster (von der Seite, rote Linie)
...
% Hilfslinien von Strahlen auf Raster und Detektor
\foreach \y in {1.5,2,2.5,3,3.5} {
    % projiziere Strahlen auf Raster
    \draw[dashed, orange] (6,\y) -- (4.5,{\y-0.1}); % Schnittpunkt Raster leicht versetzt
}
...
% Detektor-Pixelmatrix mit schwarzen Trennlinien
...
% Vertikales Streustrahlenraster (zentriert, leicht gedreht)
\begin{scope}[shift={(0.25,0.25)}, rotate=5] % Verschiebung, damit Raster mittig ist
\foreach \x in {0,0.5,...,5} {
    \draw[red, line width=0.8pt] (\x,0) -- (\x,5);
}
\end{scope}

% Horizontales Streustrahlenraster (zentriert, leicht gedreht)
...
% Legende rechts außerhalb mit mehr Abstand
...
\node at (2.5,-0.5) {Schematische Darstellung: Moiré-Muster durch Rasterüberlagerung};
