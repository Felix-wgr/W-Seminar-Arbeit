...
\usepackage{lmodern, caption, subcaption, adjustbox} 
...
\begin{figure}[H]
  \centering
  \fbox{\includegraphics[width=1\textwidth]{Bilder/Amplituden.png}}
...
werden. 
\\\\
Bei der Aufnahme der Stimmen wurde jeweils der gleiche Text gesprochen, wodurch man die Unterschiede zwischen den Stimmen gut erkennen kann. Zuerst zeige 
ich die beiden Amplituden-Diagramme der Stimmen, wobei ich jeweils einen Ausschnitt des Wortes \enquote{Hallo} aufgenommen habe.

\begin{figure}[H]
\begin{center}
\begin{tikzpicture}
    \node[anchor=south west, inner sep=0] (image) at (0,0) 
        {\fbox{\includegraphics[width=\textwidth]{Bilder/weiblich_amp.png}}};
    \begin{scope}[
        x={($ (image.south east) - (image.south west) $)},
        y={($ (image.north west) - (image.south west) $)}
    ]
        \coordinate (C) at (0.3, 0.1); 
        \coordinate (D) at (0.585, 0.9); 
        
        \fill[yellow, opacity=0.2, draw=black, line width=1pt] (C) rectangle (D);
    \end{scope}
\end{tikzpicture}
\end{center}
\caption{Ausschnitt aus dem Wort \enquote{Hallo} in der weiblichen Stimme}
\end{figure}
...
\begin{tikzpicture}
    \node[anchor=south west, inner sep=0] (image) at (0,0) 
        {\fbox{\includegraphics[width=\textwidth]{Bilder/männlich_amp.png}}};
    \begin{scope}[
        x={($ (image.south east) - (image.south west) $)},
        y={($ (image.north west) - (image.south west) $)}
    ]
        \coordinate (A) at (0.08, 0.1); 
        \coordinate (B) at (0.57, 0.9);    
        \fill[yellow, opacity=0.2, draw=black, line width=1pt] (A) rectangle (B);
    \end{scope}
\end{tikzpicture}
\end{center}
\caption{Ausschnitt aus dem Wort \enquote{Hallo} in der männlichen Stimme}
\end{figure}

Die beiden Diagramme zeigen auf den ersten Blick deutliche Unterschiede, doch bei genauerer Betrachtung erkennt man auch Gemeinsamkeiten. Denn beide 
Stimmen weisen ein ähnliches Muster auf, welches ich gelb markiert habe. Zwar stimmen die genauen Tonhöhen nicht perfekt überein, aber die Struktur 
ist ähnlich, mit dem Unterschied, dass dieses Muster bei der männlichen Stimme über einen längeren Zeitraum andauert. Das weißt darauf hin, dass die 
männliche Stimme aus kleineren Frequenzen besteht und somit tiefier klingt. Die Fourier-Transformationen beider Aufnahmen bestätigen diesen Eindruck. 

\begin{figure}[H]
  \centering
  \fbox{\includegraphics[width=\linewidth]{Bilder/weiblich_10.png}}
  \caption{Frequenzspektrum einer weiblichen Stimme}
\end{figure}

\begin{figure}[H]
  \centering
  \fbox{\includegraphics[width=\linewidth]{Bilder/männlich_10.png}}
  \caption{Frequenzspektrum einer männlichen Stimme}
\end{figure}

Anhand der beiden Frequenzspektren, oben das einer Frau, unten das eines Mannes, lassen sich nun mehrere Charakteristiken der Stimmen erkennen. Zuerst fällt auf, 
dass die Grundfrequenz, also die tiefste Frequenz in einem Gemisch harmonischer Frequenzen, bei der männlichen Stimme $(\sim 98,5 \text{Hz})$ deutlich niedriger 
ist als die der weiblichen Stimme $(\sim 173 \text{Hz})$. Bei beiden Stimmen ist auffällig, dass bei vielfachen Frequenzen der Grundfrequenz erneut starke 
Ausschläge auftreten. Sie werden als Obertöne bezeichnet und tragen maßgeblich zum Klangcharakter einer Stimme bei. Dadurch, dass die Grundfrequenz der männlichen 
Stimme niedriger ist, liegen auch die Obertöne in einem tieferen Frequenzbereich und umgekehrt bei Frauen. Die Dominaz der Obertöne bestimmt den Klang der Stimme. 
Die Höhe der Berge der Obertöne im Frequenzspektum anzupassen, entspricht einer Kompetenz eines Sängers oder einer Sängerin und dient dazu die Klangfarbe zu verändern. 
\\\\
Abgesehen von den Stimmen, sind auch ein paar Störgeräusche im Frequenzspektrum enthalten. Beispielsweise ist bei beiden Aufnahmen ein Rauschen wahrzunehmen, 
denn es ist zu erwarten, dass bei Frequenzen unter 50 Hz keine nennenswerten Anteile in einer menschlichen Stimme vorkommen. Dieses Rauschen kann durch 
Umgebungsgeräusche oder durch das Mikrofon selbst entstehen. Bei der Aufnahme der weiblichen Stimme wurde zudem eine dominate Frequenz bei ungefähr 23,5 kHz 
entdeckt, welche vermutlich durch elektronische Störquellen verursacht wurde. 

\subsection{Ausblick und Erweiterungsmöglichkeiten}
Das von mir entwickelte Programm, bildet nur eine Anwendungsmöglichkeit der Fourier-Transformation ab. Es gibt jedoch zahlreiche Möglichkeiten mein Programm 
zu erweitern und weitere Anwendungsbereiche zu erschließen. Ein möglicher nächster Schritt wäre es, das Programm mit einer einfachen künstlichen Intelligenz 
zu verbinden. Mein Ansatz für eine sehr einfache KI wäre es, die dominatesten Frequenzen einer Aufnahme in einem mehrdimensionalen Punkt eines Koordinatensystems 
zu speichern. Wenn man festlegt, wie viele Werte man speichert, erhält man einen Punkt mit einer festen Anzahl an Koordinaten. Dadurch ist es möglich die Distanz 
zwischen zwei Punkten mithilfe des Betrags des Differenzenvektors dieser Punkte zu berechnen. Mit dieser Methode lassen sich Aufnahmen mathematisch speichern 
und anschließend mit anderen Aufnahmen vergleichen. Wenn man mehrere Aufnahmen gespeichert hat, kann man so einer neuen Aufnahme die ähnlichste Aufnahme zuordnen. 
Somit könnte man beispielsweise eine Stimmerkennung implementieren, welche die Stimme einer Person erkennt, indem der Algorithmus nur überprüft, welche schon 
gespeicherte Stimme der neuen am ähnlichsten ist. Hierdurch ist dieses einefache KI-Modell auch trainierbar, indem man einfach neue Stimmen hinzufügt und so 
die Ähnlichkeitsvergleiche verbessert. 
\\\\
Je nach Anwendungsbereich könnte man so verschiedene Arten von Audiosignalen der KI füttern. Dadurch könnte man abgesehen von Stimmerkennungssystemen auch 
Musikerkennungssoftware, ähnlich wie beispielsweise Shazam, Systeme zur Erkennung von Maschinengeräuschen, um Fehler frühzeitig zu erkennen, oder auch ein 
einfaches Stimmgerät für Musikinstrumente erstellen. 
\\\\
Abgesehen davon ist eine weitere interessante Erweiterungsmöglichkeit die Dimensionen der Fourier-Transformation selbst zu erhöhen. Wodurch der Algorithmus 
auch zum Beispiel auf Bilder oder Räume angewendet werden kann. Diesen Aspekt werde ich im nächsten Abschnitt genauer erläutern.
...
\begin{figure}[H]
  ...
  \begin{minipage}{0.49\textwidth}
    \centering
    \includegraphics[width=\linewidth]{Bilder/moiré_fft.png}
...
\begin{figure}[ht]
    ...
    % Hochformat-Bild drehen, oben bündig
    \raisebox{0pt}[0pt][0pt]{\adjustbox{angle=90,width=0.62\textwidth}{\includegraphics{Bilder/Detektor.jpg}}}
    \caption{Links: Streustrahlenraster; Rechts: Detektor mit Streustrahlenrastern (unten)}
\end{figure}

Stark vereinfacht kann man das im Modell wie folgt skizzieren:
\\\\
\noindent
\begin{figure}[ht]
\centering
...
% Röntgenquelle (links)
\draw[fill=yellow] (0.5,2.5) circle (0.2);
\node[left] at (0,2.5) {Röntgenquelle};

% Primärstrahlen
\foreach \y in {1.5,2,2.5,3,3.5} {
    \draw[green!70!black, thick,->] (0.5,2.5) -- (6,\y);
}

% Mensch (Objekt)
\draw[thick, gray] (3,1.8) -- (3,3.2);
\node[above right] at (2.2,3.2) {Mensch};

% Streustrahlenraster (rote Linie)
\draw[red, thick] (4.5,1) -- (4.5,4);
\node[right] at (4.5,4) {Raster};

% Detektorfläche
\draw[fill=blue!20] (6,1) rectangle (6.2,4);
\node[right] at (6.2,4) {Detektor};

% Streustrahlung vom Mensch zum Raster (wellig, grün)
\foreach \y in {1.9,2.2,2.6,2.9,3.1} {
    % Wellig bis Raster
    \draw[green!70!black, decorate, decoration={snake, amplitude=0.05cm, segment length=0.2cm}, ->] 
        (3,\y) -- (4.5,{\y+0.05});
        
    % Gerade nach Raster geordnet zum Detektor
    \pgfmathsetmacro{\yRaster}{round((\y+0.05-1)/0.5)*0.5 + 1} % Auf Rasterlinien runden
    \draw[green!70!black, thick, ->] (4.5,{\y+0.05}) -- (6,\yRaster);
}
\end{tikzpicture}
\caption{Vereinfachtes Modell eines Röntgengeräts mit Streustrahlenraster und Detektor}
\end{figure}

Wenn die Streustrahlenraster nicht perfekt mit dem Detektor abgestimmt sind, dann erhält man eine Überlagerung von regelmäßigen Strukturen. 
Dadurch können Moiré Muster entstehen. Stark vereinfacht kann man sich das so vorstellen: 
\noindent
\begin{figure}[ht]
\centering
\begin{tikzpicture}[scale=0.8]
% Detektor-Pixelmatrix
\foreach \x in {0,0.5,...,5} {
    \foreach \y in {0,0.5,...,5} {
        \fill[blue!20] (\x,\y) rectangle (\x+0.5,\y+0.5);
        \draw[black] (\x,\y) rectangle (\x+0.5,\y+0.5);
    }
}

% Vertikales Streustrahlenraster
\begin{scope}[shift={(0.25,0.25)}, rotate=5]
\foreach \x in {0,0.5,...,5} {
    \draw[red, line width=0.8pt] (\x,0) -- (\x,5);
}
\end{scope}

% Horizontales Streustrahlenraster
\begin{scope}[shift={(0.25,0.25)}, rotate=5]
\foreach \y in {0,0.5,...,5} {
    \draw[red, line width=0.8pt] (0,\y) -- (5,\y);
}
\end{scope}

% Legende
\draw[fill=blue!20] (6.5,4) rectangle (7,4.5);
\node[right] at (7,4.25) {Detektor-Pixel};
\draw[red, line width=0.8pt] (6.5,3) -- (7,3);
\node[right] at (7,3) {Streustrahlenraster};
\end{tikzpicture}
\caption{Vereinfachte Darstellung eines Detektors mit schlecht ausgerichtetem Streustrahlenraster}
\end{figure}
