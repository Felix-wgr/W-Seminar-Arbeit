...
\usepackage{amsmath, amssymb, amsthm, graphicx, pgfplots, tikz, forest, xcolor, listings}
\usepackage{lmodern} 
...
\definecolor{BGGray}{RGB}{40,40,40}       % Hintergrund
\definecolor{KeywordBlue}{RGB}{86,156,214}       % Kontrollstrukturen
\definecolor{TypeTurquoise}{RGB}{78,201,176}    % Datentypen
\definecolor{NamespacePurple}{RGB}{180,122,180} % Namespaces/Objekte
\definecolor{CommentGreen}{RGB}{87,166,74}      % Kommentare
\definecolor{StringOrange}{RGB}{206,145,120}    % Strings
\definecolor{NumberGray}{RGB}{181,206,168}      % Zahlen
\definecolor{TextGray}{RGB}{220,220,220}        % Normale Schrift

% C++ Sprache für Listen
\lstdefinelanguage{CPPVS}{
    language=C++,
    morekeywords=[1]{int, float, double, short, long, char, void, bool},
    morekeywords=[2]{for, if, else, return, new},
    morekeywords=[3]{std, complex},
    sensitive=true,
    morecomment=[l]{//},
    morestring=[b]"
}
...
\lstdefinestyle{VSStyle}{
    ...
    keywordstyle=[1]\color{TypeTurquoise}\bfseries,     % Datentypen
    keywordstyle=[2]\color{KeywordBlue}\bfseries,       % Kontrollstrukturen
    keywordstyle=[3]\color{NamespacePurple},            % Namespaces/Objekte
...
\\\\
\begin{center}
\includegraphics[scale=0.825]{DFT_Code.jpg}
\end{center}
...
verwenden, die jeweils nur halb so groß wie die ursprüngliche Liste sind. Dies führt zu einer Vereinfachung des Faktors 2 im 
Exponenten der Basis $e$. So wird die Formel im Programm verwendet: 
\[
\sum_{j=0}^{N-1}{f(j)\cdot e^{-i\omega j}} = e^{-i\omega}\sum_{j=0}^{N_{\text{Neu}}-1}{f_{\text{ungerade}}(j)\cdot e^{-i \omega j}} + \sum_{j=0}^{N_{Neu}-1}{f_{\text{gerade}}(j)\cdot e^{-i \omega j}}
\]
\\
