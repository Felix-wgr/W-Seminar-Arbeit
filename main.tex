...
\usepackage{amsmath, amssymb, amsthm, graphicx, pgfplots, tikz, forest, xcolor, lstlinebgrd, mdframed}
\usepackage{lmodern} 
...
\[
\sum_{j=0}^{N-1}{f(j)\cdot e^{-i\omega j}} = e^{-i\omega}\sum_{j=0}^{N_{\text{Neu}}-1}{f_{\text{ungerade}}(j)\cdot e^{-i \omega j}} + \sum_{j=0}^{N_{Neu}-1}{f_{\text{gerade}}(j)\cdot e^{-i \omega j}} \quad \text{mit $N_{\text{Neu}} := N/2$ }
\]
\\
...
\[
\sum_{j=0}^{N-1}{f(j)\cdot e^{-i \frac{2\pi k}{N} j}} = e^{-i \frac{2\pi k}{N_{\text{neu}}}} \sum_{j=0}^{N_{\text{Neu}}-1}{f_{\text{ungerade}}(j)\cdot e^{-i \frac{2\pi k}{N_{\text{neu}}} j}} + \sum_{j=0}^{N_{Neu}-1}{f_{\text{gerade}}(j)\cdot e^{-i \frac{2\pi k}{N_{\text{neu}}} j}}
\]
...
(std::vector<float>& function, int size) {
  ...
  (*@\colorbox{blue!30}{std::complex<float> twiddle = \hspace*{17,5em}}@*)
  (*@\colorbox{blue!30}{std::exp(-2.0f * PI\_F * imaginary * (float)k / (float)size);\hspace*{2,5em}}@*)
  (*@\colorbox{blue!30}{result[k] = even[k] + twiddle * odd[k];\hspace*{13em}}@*)
  (*@\colorbox{blue!30}{result[k + size / 2] = even[k] - twiddle * odd[k];\hspace*{7,5em}}@*)
